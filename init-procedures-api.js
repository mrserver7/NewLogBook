...
/**
 * Initialize comprehensive procedures via API
 */
...
async function initializeProceduresViaAPI() {
  const baseURL = 'http://localhost:5000';
  
  console.log('Initializing comprehensive procedures via API...');
  
  try {
    // First, try to call the init-procedures endpoint
    const initResponse = await fetch(`${baseURL}/api/init-procedures`, {
      method: 'POST',
      headers: {
    ...
    if (!initResponse.ok) {
      console.log('Init endpoint not available, creating procedures individually...');
      ...
      for (const procedure of COMPREHENSIVE_PROCEDURES) {
        try {
          ...
          if (response.ok) {
            ...
            console.log(`✗ Failed to create ${procedure.name}: ${response.statusText}`);
          }
        } catch (error) {
          console.log(`✗ Failed to create ${procedure.name}: ${error.message}`);
        }
      }
    } else {
      const result = await initResponse.json();
      console.log('✓ Initialized via bulk endpoint:', result);
    }

    // Verify by fetching all procedures
    const fetchResponse = await fetch(`${baseURL}/api/procedures`);
    if (fetchResponse.ok) {
      const procedures = await fetchResponse.json();
    ...
    console.error('Error initializing procedures:', error.message);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  initializeProceduresViaAPI().catch(console.error);
}

export { COMPREHENSIVE_PROCEDURES, initializeProceduresViaAPI };